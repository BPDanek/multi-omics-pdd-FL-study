...
def run_baseline_exp(write_results_path, fold_idx):
    ...
    # performs logging
    hp_logs = run_baseline_experiments(current_experiment, fold_idx)
    # return hp_logs
    write_paths = current_experiment.write_raw_experiment_results_as_df(fold_idx, write_results_path)
    print(write_paths)
...
def delete_dir():
    dir =  '/home/bdanek2/federated_learning_multi_modality_ancestry/multi_modality_fl/experiments'
...
def run_series(fold_idx: int):
    ...
    write_results_path=os.path.join(repo_parent, 'federated_learning_multi_modality_ancestry/multi_modality_fl/results/red_fedlogreg_uniform_strat/')
    delete_dir()
    ...
    stratified = False
    # records experiment results
    write_results_path=os.path.join(repo_parent, 'federated_learning_multi_modality_ancestry/multi_modality_fl/results/red_fedlogreg_uniform_non_strat/')
    delete_dir()
    ...
    stratified = False
    # records experiment results
    write_results_path=os.path.join(repo_parent, 'federated_learning_multi_modality_ancestry/multi_modality_fl/results/red_fedlogreg_linear_non_strat/')
    delete_dir()
    run_baseline_exp(write_results_path, fold_idx) # next
    delete_dir()
    run_exp(write_results_path, repo_parent, fold_idx, num_round, num_local_rounds, client_lr, 2, split_method, stratified)
...
overall_external = []
hp = run_series(0)
# overall_internal.extend(hp[0])
# overall_external.extend(hp[1])
hp = run_series(1)
# overall_internal.extend(hp[0])
# overall_external.extend(hp[1])
hp = run_series(2)
...
# pickle.dump(overall_internal, open(os.path.join("/tmp/", 'hyper-param-cent-internal'), 'wb'))
# pickle.dump(overall_external, open(os.path.join("/tmp/", 'hyper-param-cent-external'), 'wb'))